...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Pin CategoryBar when header has scrolled out of view (typically ~100px due to more compact header)
    ...
    <PageTransition>
      <div className="flex flex-col">
        ...
        >
          ...
          <div className="flex justify-between items-center gap-2 mb-1 relative z-10">
            ...
            <motion.button 
              ...
              whileTap={{ scale: 0.9 }}
              className="p-2 bg-primary text-white rounded-xl shadow-lg shadow-primary/20 hover:shadow-primary/30 border border-white/15 active:scale-90 transition-all flex items-center justify-center shrink-0"
            ...
            >
              ...
              <div className="w-8 h-8 bg-gradient-to-br from-primary to-[#E89C6A] rounded-[16px] flex items-center justify-center text-white shadow-lg shadow-primary/20 group-hover:rotate-12 transition-transform duration-500 shrink-0">
                <Hash size={14} strokeWidth={3} />
              </div>
            </motion.div>

            <div className="relative shrink-0 flex gap-2">
               <button onClick={() => navigate('/cafe-info')} className="p-2 bg-gray-100 dark:bg-white/10 rounded-xl text-dark dark:text-white backdrop-blur-md border border-gray-200 dark:border-white/5 active:scale-90 transition-transform">
                 <Info size={18} />
               </button>
               <div className="relative">
                 <button onClick={() => navigate('/messages')} className="p-2 bg-gray-100 dark:bg-white/10 rounded-xl text-dark dark:text-white backdrop-blur-md border border-gray-200 dark:border-white/5 active:scale-90 transition-transform">
                   <Bell size={18} />
                 </button>
                 <span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 bg-primary rounded-full border border-white dark:border-dark"></span>
               </div>
            </div>
          </div>

          <div className="text-dark dark:text-white mb-0.5 relative z-10 text-right">
            <h1 className="text-[10px] text-muted font-black tracking-[2px] mb-0.5">کافه لند</h1>
            <p className="text-lg font-black leading-tight">{getGreeting()} وقت یه قهوه خوبه! ☕</p>
          </div>

          {/* نوار وضعیت تعاملی کافه جهت اطلاع و همچنین شبیه‌سازی تست */}
          <div className="relative z-10 flex items-center justify-between mt-0.5 pt-2 border-t border-gray-100/40 dark:border-white/5" dir="rtl">
