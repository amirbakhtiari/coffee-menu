...
const OTPInput: React.FC<OTPInputProps> = ({ value, onChange, length = 4 }) => {
  ...
  const handleChange = (index: number, val: string) => {
    ...
    if (val.length > 1) {
      const sliced = val.slice(0, length - index).split('');
    ...
    <div className="flex gap-4 ltr" dir="ltr">
      {Array.from({ length }).map((_, idx) => (
        <input
          ...
          className="w-14 h-16 bg-white dark:bg-black/20 border-2 border-gray-100 dark:border-white/10 rounded-2xl text-center text-xl font-black text-primary focus:border-primary focus:ring-4 focus:ring-primary/10 outline-none transition-all shadow-lg shadow-black/5 dark:shadow-none"
        />
      ))}
    </div>
  );
